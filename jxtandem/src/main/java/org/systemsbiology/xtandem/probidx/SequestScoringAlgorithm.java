...
/**
 * org.systemsbiology.xtandem.probidx.SequestScoringAlgorithm
 * User: Steve
 * Date: 1/12/12
 */
public class SequestScoringAlgorithm extends AbstractScoringAlgorithm  {

    public static final SequestScoringAlgorithm[] EMPTY_ARRAY = {};
    ...
    public static final MassType DEFAULT_MASS_TYPE = MassType.monoisotopic;

    public static final String ALGORITHM_NAME = "Sequest";

    private MassType m_MassType = DEFAULT_MASS_TYPE;
    private double m_PeptideError = DEFAULT_PEPTIDE_ERROR;

    public SequestScoringAlgorithm() {
...
//    }
    ...
    public static final double BOUNS_FACTOR_FOR_CONSECUTIVE_MATCH = 0.075;
    /**
     * BONUS - consecutice matched fragment
     * any time there is a match for 2 consecutive IONS - Y -- Y or B-- B
     * For all charges
     * preliminary score is multiplied by 1 + Bouns computed for ALL charge states
     * @return
     */
    ...
    /**
     *  Sum of matched intensities * (Matched IOns / TotalIOns) (1 + Bonus) /
     *
     * @param measured !null measured spectrum
     * @param theory   !null theoretical spectrum
     * @param otherdata any other data needed
     * @return value of the score
     */
      public double preliminaryScoreSpectrum(final IMeasuredSpectrum measured, final ITheoreticalSpectrum theory, Object... otherdata) {
    ...
    /*
       In Normalizing - take sqrt but  bin data 10 windows
       windows lowest to highest mass 10 windows
       scale window intensity = 50 in window scale to window
        do not divide by sum square intensities

        In Final score - after normalizing by sqrt but before windowing
           find+/- 50 daltons of peak - only keep   if 1 sd above average - before windowing
          make 2 passes keeping some survivors at least 1 sd above mean in +/- 50 D window


           dot_product Score is 50 for same bin 25 for adjacent bin user may add -17 NH3 -18 H20 score 10
           user selectable


           Perform mix-range modification to input spectrum -
           in ORIGINAL K score is equivalent of taking 50 dot droducts and taking mean where / 101 is
           +/= 5o offset  Seaquest does +/- 75
            add all peaks +/- 75 Subtract mean of +/- -

            in Seaquest if a B and Y ions end up in the same bin you do not count twice
    ...
    public void setMassType(final MassType pMassType) {
    ...
    protected IMeasuredSpectrum truncatePeakList(final MutableMeasuredSpectrum in) {
        ...
        MutableMeasuredSpectrum out = new MutableMeasuredSpectrum();
    ...
    /**
     * NOTE only for preliminary scoring !!!!! for final score more similar to K score
     * 1) Place in 1.0005=dalton bins  choose Max value in any bin
     * 2) Choose top 200 bins
     * 3) Any set of adjacent bins gets the max - i.e. 501,502,503,504 ... all get the max
     *  4) Global max peak height = 100
     * @param in
     * @return
     */
