...
/**
 * org.systemsbiology.xtandem.fragmentation.Blosum
 * User: steven
 * Date: 6/28/12
 */
import java.lang.*;
    ...
    */
    ...
    private static int getIndex(char a) {
    	...
    	switch ((String.valueOf(a)).toUpperCase().charAt(0)) {
	    	...
	    	case 'V': return 19;
	    	default:
                throw new IllegalArgumentException( "Invalid amino acid character" + a);
    	}
    }

    private static int getDistance(int i, int j) {
    	if (i < 0 || i > matrix[0].length) {
    		throw new IllegalArgumentException("Invalid amino acid character at string1, position " + i);
    	}
    	if (j < 0 || j > matrix[0].length) {
    		throw new IllegalArgumentException("Invalid amino acid character at string2, position " + j);
    	}
