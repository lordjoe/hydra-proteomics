...
public class BiomlReporter {
    ...
    public BiomlReporter(XTandemMain pParameters, OutputStream pOut) {
        this(pParameters, pParameters.getScoreRunner(), pOut);
    }
    ...
    public void writeReportEnd(PrintWriter pOut) {
        if (isParametersShown()) {
            writeParameters(pOut, 0);
            writeUnusedParameters(pOut, 0);
        }
        if (isPerformanceShown())
            writePerformanceParameters(pOut, 0);
        writeFooter(pOut, 0);
    }


    protected void indent(PrintWriter out, int indent) {
    ...
    public void writeHeader(PrintWriter out, int indent) {
        out.println("<?xml version=\"1.0\"?>");
        IParameterHolder ph = getParameters();
        String xsl = ph.getParameter("output, xsl path");
        if (xsl != null)
            out.println("<?xml-stylesheet type=\"text/xsl\" href=\"" + xsl + "\"?>");
        ...
        // note it is from us
        out.println("<!-- generated by jxtandem -->");
     }
    ...
    public void writeScanScores(IScoredScan scan, PrintWriter out, int indent) {
        ...
        // note it is from us
        out.println("<!--   sequence=" + polypeptide.toString() +  " -->");

        // todo handle m_ParentStream protein
        final IProtein parentProtein = null; // polypeptide.getParentProtein();
        indent(out, indent);
        out.print("<group ");
        ...
        out.print("maxI=\"" + XTandemUtilities.formatDouble(maxIntensity, 1) + "\" ");
        out.print("fI=\"" + XTandemUtilities.formatDouble(maxIntensity / 100, 3) + "\" ");
      ...
      //      out.print("uid=\"" + parentProtein.getUUid() + "\" ");
        ...
        indent(out, indent);
        out.print("<file ");
        out.print("type=\"" + "peptide" + "\" ");
        if (parentProtein != null) {
            out.print("URL=\"" + parentProtein.getURL() + "\" ");
        }
        out.println(" />");
        ...
        out.print("start=\"1\" ");
        if (parentProtein != null)
            out.print("end=\"" + parentProtein.getSequenceLength() + "\" ");
        else
            out.print("end=\"" + polypeptide.getSequenceLength() + "\" ");   // ok there is no protein
...
//        out.println("</group>");
    ...
    protected void outputHistogram(IScoredScan scan, PrintWriter out, int indent) {
        RawPeptideScan raw = scan.getRaw();
        ...
        int numvalues = 50;
        indent(out, indent);
        out.println("<group type=\"support\" label=\"fragment ion mass spectrum\">");
        IParameterHolder ph = getParameters();
        String title = ph.getParameter("output, title");
        if (title == null)
            title = ph.getParameter("spectrum, path");
        ...
        out.println("</GAML:trace>");
        out.println("</group></group>");
    }

    private void outputSpectrumData(final PrintWriter out, final int indent, IScoredScan scan) {

        IMeasuredSpectrum cs = scan.getConditionedScan();
        ISpectrumPeak[] peaks = cs.getPeaks();

        indent(out, indent);
        out.print("<GAML:Xdata ");
        out.print("label=\"" + scan.getId() + ".spectrum" + "\" ");
        out.print("units=\"" + "MASSTOCHARGERATIO" + "\" ");
        out.println(">");
        indent(out, indent);
        out.println(
        ...
        for (int i = 0; i < peaks.length; i++) {
            ...
            out.print(XTandemUtilities.formatDouble(mass, 3));
            if ((i + 1) % 30 == 0) {
                indent(out, indent);
                out.println();
            }
        ...
        out.println();
        indent(out, indent);
        out.println("</GAML:values>");
        indent(out, indent);
        out.println("</GAML:Xdata>");

        indent(out, indent);
        out.print("<GAML:Ydata ");
        out.print("label=\"" + scan.getId() + ".spectrum" + "\" ");
        out.print("units=\"" + "UNKNOWN" + "\" ");
        out.println(">");
        indent(out, indent);
        out.println(
                "<GAML:values byteorder=\"INTEL\" format=\"ASCII\" numvalues=\"" + peaks.length + "\">");
        ...
        out.println("</GAML:values>");
        indent(out, indent);
        out.println("</GAML:Ydata>");
    }

    protected void outputDomain(IScoredScan scan, PrintWriter out, int indent) {
        RawPeptideScan raw = scan.getRaw();
        ...
        out.print("start=\"0\" ");   // not supported
        out.print("end=\"" + polypeptide.getSequenceLength() + "\" ");   // not supported

        out.print("expect=\"" + XTandemUtilities.formatScientific(expect, 1) + "\" ");
 ...
 //       double mPlusHPeptide = mass + XTandemUtilities.getProtonMass() + XTandemUtilities.getCleaveCMass() + XTandemUtilities.getCleaveNMass();  // M + h
        ...
        out.print("mh=\"" + XTandemUtilities.formatDouble(mass, 3) + "\" ");
        double diff = del;
        out.print(
        ...
        if (nextmatch != null)
            out.print("nextscore=\"" + XTandemUtilities.formatDouble(nextmatch.getHyperScore(),
                    0) + "\" ");
        else
            out.print("nextscore=\"100\" "); // just copying the XTandem behavior
        ...
        //      out.print("pre=\"" + polypeptide.getPreSequence(4) + "\" ");
        //      out.print("post=\"" + polypeptide.getPostSequence(4) + "\" ");


        out.print("pre=\"\" ");   // not supported
        out.print("post=\"\" ");   // not supported

        out.print("seq=\"" + polypeptide.getSequence() + "\" ");
        out.print("missed_cleavages=\"" + polypeptide.getMissedCleavages() + "\" ");
        out.println(" >");
    ...
    protected void outputModifications(final IModifiedPeptide mp, final PrintWriter out, int indent) {
    ...
    protected void outputModification(final IModifiedPeptide mp,PeptideModification modification ,int index, final PrintWriter out, int indent) {
        indent(out, indent);
        out.print("<aa ");
    ...
    protected void outputIonTypeScores(PrintWriter out, ISpectralMatch pBestMatch) {
        out.print("y_score=\"" + XTandemUtilities.formatDouble(pBestMatch.getScore(IonType.Y),
                0) + "\" ");
        out.print("y_ions=\"" + pBestMatch.getCount(IonType.Y) + "\" ");

        out.print("b_score=\"" + XTandemUtilities.formatDouble(pBestMatch.getScore(IonType.B),
                0) + "\" ");
        out.print("b_ions=\"" + pBestMatch.getCount(IonType.B) + "\" ");

        out.print("c_score=\"" + XTandemUtilities.formatDouble(pBestMatch.getScore(IonType.C),
                2) + "\" ");
        out.print("c_ions=\"" + pBestMatch.getCount(IonType.C) + "\" ");

        out.print("z_score=\"" + XTandemUtilities.formatDouble(pBestMatch.getScore(IonType.Z),
                2) + "\" ");
        out.print("z_ions=\"" + pBestMatch.getCount(IonType.Z) + "\" ");

        out.print("a_score=\"" + XTandemUtilities.formatDouble(pBestMatch.getScore(IonType.A),
                2) + "\" ");
        out.print("a_ions=\"" + pBestMatch.getCount(IonType.A) + "\" ");

        out.print("x_score=\"" + XTandemUtilities.formatDouble(pBestMatch.getScore(IonType.X),
                2) + "\" ");
        out.print("x_ions=\"" + pBestMatch.getCount(IonType.X) + "\" ");
    }

    protected void writeDescriptionNote(String text, PrintWriter out, int indent) {

        writeDescriptionNote(text, out, indent, true);

    }

    protected void writeDescriptionNote(String text, PrintWriter out, int indent, boolean onTwoLines) {
    ...
    protected void writeSupportingData(PrintWriter out, int indent, int id, double hyperA0,
                                       double hyperA1) {
        indent(out, indent);
        out.println("<group label=\"supporting data\" type=\"support\">");
        indent(out, indent);
        out.println(
    ...
    protected void writeGAMLAttribute(PrintWriter out, int indent, String type, String value) {
        indent(out, indent);
        out.println("<GAML:attribute type=\"" + type + "\">" + value + "</GAML:attribute>");

    }

    protected void writeSupportingValues(PrintWriter out, int indent, int id, IScoredScan scan) {
        IMeasuredSpectrum cs = scan.getConditionedScan();
        ISpectrumPeak[] peaks = cs.getPeaks();
        indent(out, indent);
        out.println("<GAML:Xdata label=\"" + id + ".hyper\" units=\"score\">");
        indent(out, indent);
        out.println(
                "<GAML:values byteorder=\"INTEL\" format=\"ASCII\" numvalues=\"" + peaks.length + "\">");
        indent(out, indent);
        ...
        indent(out, indent);
        out.println("<GAML:Ydata label=\"" + id + ".hyper\" units=\"counts\">");
        indent(out, indent);
        out.println(
                "<GAML:values byteorder=\"INTEL\" format=\"ASCII\" numvalues=\"" + peaks.length + "\">");
        for (int i = 0; i < peaks.length; i++) {
    ...
    protected void writeBytes(PrintWriter out, int indent, byte[] xdata1) {
        int colWidth = getHistogramColumnWidth();
        int column = 0;
        DecimalFormat fmt = new DecimalFormat("000");
        for (int i = 0; i < xdata1.length; i++) {
            int b = xdata1[i];
            out.print(fmt.format(b));
            column += 4;
            if (column > colWidth) {
                out.println();
                indent(out, indent);
    ...
    protected void writeParameter(String key, PrintWriter out, int indent) {
        final String value = getParameters().getParameter(key);
        if (value != null)
            writeNote(key, value, out, indent);
    }

    protected void writePerformanceParameter(String key, PrintWriter out, int indent) {
     ...
     */
    protected void writeNote(String label, String value, PrintWriter out, int indent) {
        indent(out, indent);
        out.print("<note type=\"input\"  label=\"" + xmlEscape(label) + "\" >");
        out.print(xmlEscape(value));
        out.println("</note>");
    }
     ...
     * @param indent indent if any
     */
    protected void writeNote(String value, PrintWriter out, int indent) {
        indent(out, indent);
        out.print("<note >");
        out.print(xmlEscape(value));
        out.println("</note>");
    ...
    protected void writeFooter(PrintWriter out, int indent) {
        indent(out, indent);
        out.println("</bioml>");
    }
 ...
 */
    ...
    private void writePerformanceParameters(PrintWriter pOut, int indent) {
        String[] params = {
